...
export default function SocialMedia() {
  const reels = ['/assets/videos/reel3.mp4', '/assets/videos/reel1.mp4', '/assets/videos/reel2.mp4'];
  const [playing, setPlaying] = useState('');

  return (
    <section id="work" className="py-16 lg:py-28 bg-accent-1/5 px-6">
      <div className="max-w-5xl mx-auto">
        ...
        >
          <img src="/assets/logos/instagram.png" alt="Instagram logo" className="h-12 w-12" />
          spence_ontherocks
        </a>
        <div className="flex flex-wrap justify-center items-center gap-12">
          {reels.map((reel, index) => (
            <InstagramEmbed src={reel} key={`reel${index}`} playing={playing} setPlaying={setPlaying} />
          ))}
        </div>
      </div>
    </section>
